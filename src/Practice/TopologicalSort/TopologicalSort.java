...
public class TopologicalSort {
    ...
    public static List<Integer> topSort(Graph graph) {
        ...
        vertices.stream().filter(vertex -> !visited.contains(vertex)).forEach(vertex -> topSortUtil(visited, result, vertex, graph));
        return result;
    }
    ...
    private static void topSortUtil(Set<Integer> visited, Stack<Integer> result, Integer vertex, Graph graph) {
        ...
        if (graph.adjacencyList[vertex] != null) {
            graph.adjacencyList[vertex].stream().filter(v -> !visited.contains(v)).forEach(v -> topSortUtil(visited, result, v, graph));
        }
